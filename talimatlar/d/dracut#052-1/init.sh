...
if [ "$?" != "0" ]; then
    echo "Proc /proc'a bağlanamıyor! Çekirdeği CONFIG_PROC_FS ile derleyin!"
    exit 1
fi
...
if [ "$?" != "0" ]; then
    echo "/sys üzerine sysfs bağlanamıyor! Çekirdeği CONFIG_SYSFS ile derleyin!"
    exit 1
fi
...
if ! ismounted /dev; then
    echo "/dev üzerine devtmpfs bağlanamıyor! Çekirdeği CONFIG_DEVTMPFS ile derleyin!"
    exit 1
fi
...
if getarg "rd.cmdline=ask"; then
    echo "Ek çekirdek komut satırı parametresini girin (ctrl-d veya ile bitirin.) "
...
while :; do
    ...
    for f in $hookdir/mount/*.sh; do
        ...
        if ismounted "$NEWROOT"; then
            usable_root "$NEWROOT" && break;
            warn "$NEWROOT uygun bir rootfs düzenine sahip değil, rahatsız edici montaj kancas\u0131n\u0131 yok say\u0131yor ve kald\u0131r\u0131yor"
            umount "$NEWROOT"
            rm -f -- "$f"
        fi
    done
    ...
    _i_mount=$(($_i_mount+1))
    [ $_i_mount -gt 20 ] \
        && { flock -s 9 ; emergency_shell "Kök dosya sistemi bağlanam\u0131yor"; } 9>/.console_lock
done

{
    printf "Bağlı kök dosya sistemi "
...
cp init /usr/bin/
[ "$INIT" ] || {
    echo "\u0130nit bulunam\u0131yor!"
    echo "L\u00FCtfen geçerli bir k\u00F6k dosya sistemi ge\u00E7ti\u011Finizden emin olun!"
    emergency_shell
}
